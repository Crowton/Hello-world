...
public class GenericMergeSortInPlace {
    ...
    private static <T> void mergeRange(T[] arr, int from, int middle, int to, Comparator<T> comparator) {
        ...
        for (int k = from; k < to; k++) {
            // All in the left array is taken - the right will already be at the correct place
            ...
            if (comparator.compare((T) left[leftTaken], arr[from + leftSize + rightTaken]) <= 0) {
                arr[k] = (T) left[leftTaken];
                leftTaken++;
            } else {
                arr[k] = arr[from + leftSize + rightTaken];
                rightTaken++;
            }
        }
    }
